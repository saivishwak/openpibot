...
class PI05InferenceOptions:
    """Typed configuration for embedded PI0.5 inference.

    CLI wrappers should parse arguments into this object before constructing
    `PI05InferenceRuntime`. The runtime still normalizes to an argparse
    namespace internally so the standalone script and embedded server share the
    same control path.
    """
    ...
    stop_on_episode_error: bool = False
    device: Literal["cuda", "cpu", "mps"] | str = "cuda"
    fps: int = 30
    action_horizon: int = 50
    open_loop_steps: int = 35
    replan_on_miss_deg: float = 0.0
    replan_miss_steps: int = 2
    settle_steps: int = 60
    replan_blend: float = 0.2
    phase1_task: str | None = None
    phase1_sec: float = 0.0
    strict_motors: bool = False
    skip_home: bool = False
    home_before_episode: bool = True
    skip_home_after_episode: bool = False
    home_timeout: float = 60.0
    max_relative_target: float | None = None
    gripper_max_relative_target: float | None = None
    action_shape_kp: float = _DEFAULT_ACTION_SHAPE_KP
    policy_ema_alpha: float = 0.36
    command_ema_alpha: float = 0.2
    joint_deadband_deg: float | None = None
    clamp_to_present: bool = True
    dry_run: bool = False
    dry_run_home: bool = False
    show_cameras: bool = False
    preview_fps: float = 15.0
    camera_backend: Literal["dashboard", "lerobot"] | str = "dashboard"
    print_startup_summary: bool = True

    def to_namespace(self) -> argparse.Namespace:
        data = dataclasses.asdict(self)
        data["policy_path"] = str(pathlib.Path(self.policy_path).expanduser().resolve())
        return argparse.Namespace(**data)


def inference_options_from_args(args: argparse.Namespace) -> PI05InferenceOptions:
    """Convert parsed CLI/runtime args into typed embedded runtime options."""

    return PI05InferenceOptions(
        policy_path=args.policy_path,
        task=args.task,
        episodes=int(args.episodes),
        episode_time=int(args.episode_time),
        episode_steps=args.episode_steps,
        stop_on_episode_error=bool(args.stop_on_episode_error),
        device=str(args.device),
        fps=int(args.fps),
        action_horizon=int(args.action_horizon),
        open_loop_steps=int(args.open_loop_steps),
        replan_on_miss_deg=float(args.replan_on_miss_deg),
        replan_miss_steps=int(args.replan_miss_steps),
        settle_steps=int(args.settle_steps),
        replan_blend=float(args.replan_blend),
        phase1_task=args.phase1_task,
        phase1_sec=float(args.phase1_sec),
        strict_motors=bool(args.strict_motors),
        skip_home=bool(args.skip_home),
        home_before_episode=bool(args.home_before_episode),
        skip_home_after_episode=bool(args.skip_home_after_episode),
        home_timeout=float(args.home_timeout),
        max_relative_target=args.max_relative_target,
        gripper_max_relative_target=args.gripper_max_relative_target,
        action_shape_kp=float(args.action_shape_kp),
        policy_ema_alpha=float(args.policy_ema_alpha),
        command_ema_alpha=float(args.command_ema_alpha),
        joint_deadband_deg=args.joint_deadband_deg,
        clamp_to_present=bool(args.clamp_to_present),
        dry_run=bool(args.dry_run),
        dry_run_home=bool(args.dry_run_home),
...
def _skip_post_home_after_episode(args: argparse.Namespace, stop_reason: str | None) -> bool:
    if bool(args.skip_home_after_episode):
        return True
    return stop_reason == "interrupted by operator"
...
class PI05InferenceRuntime:
    """Long-lived PI0.5 runtime for serving multiple task prompts without reloads."""

    def __init__(self, args: argparse.Namespace | PI05InferenceOptions) -> None:
        self.cfg = _load_yaml()
        if isinstance(args, PI05InferenceOptions):
            args = args.to_namespace()
        self.args = args
        self.vr_joint_caps, self.vr_joint_deadbands = _load_vr_control_shaping(self.cfg)
        self.homing_present_tol, self.homing_soft_stall_tol = _load_homing_tolerances(
            self.cfg
        )
        self.action_shape_kp = max(0.0, min(1.0, float(args.action_shape_kp)))
        self.effective_max_rel = _effective_max_relative_target(args, self.cfg)
        if args.joint_deadband_deg is not None:
            override_deadband = max(0.0, min(5.0, float(args.joint_deadband_deg)))
            self.vr_joint_deadbands = {
                joint: (0.0 if joint == "gripper" else override_deadband)
                for joint in self.vr_joint_deadbands
            }
        self.policy_path = pathlib.Path(args.policy_path).resolve()
        if not self.policy_path.is_dir():
            raise FileNotFoundError(f"policy path not found: {self.policy_path}")

        if bool(getattr(args, "print_startup_summary", True)):
            print("=" * 60)
            print("  Mode              : warm PI0.5 runner")
            print(f"  Policy checkpoint : {self.policy_path}")
            print(f"  Episodes          : {args.episodes} x <= {args.episode_time}s @ {args.fps} fps")
            print(f"  Camera backend    : {args.camera_backend}")
            print(f"  Device            : {args.device}")
            print("=" * 60)

        _run_motor_port_preflight(self.cfg)
        self.home_pose = _read_home_pose(self.cfg) if not args.skip_home else {}

        _ensure_xlerobot_import()
        from _camera_preview_window import CameraPreviewWindow
        from _camera_utils import print_inference_camera_map
        from _dashboard_camera_session import (
            attach_dashboard_cameras_to_robot,
            detach_dashboard_cameras_from_robot,
        )
        ...
        self.detach_dashboard_cameras_from_robot = detach_dashboard_cameras_from_robot
        self.predict_action = predict_action
        self.precise_sleep = precise_sleep

        self.use_dashboard_cameras = args.camera_backend == "dashboard"
        if not self.use_dashboard_cameras:
            print(
                "Note: --camera-backend lerobot opens 3 V4L captures on the robot; "
                "prefer --camera-backend dashboard unless required."
            )
        patch_xlerobot_motors_only_connected()
        if not self.use_dashboard_cameras:
        ...
        if args.max_relative_target is not None or args.gripper_max_relative_target is not None:
            self.cfg.setdefault("robot", {})["max_relative_target"] = self.effective_max_rel

        self.device = torch.device(args.device)
        self.policy = _load_pi05_policy_with_compat(self.policy_path, self.device)
        _verify_policy_checkpoint(self.policy_path, self.policy)
        self.policy_joint_names = list(self.policy.config.action_feature_names or [])
        self.normalized_policy_joint_names = [
            _normalize_joint_name(name) for name in self.policy_joint_names
        ]
        if len(self.normalized_policy_joint_names) != 12:
            raise RuntimeError(
                f"expected 12 action joints in checkpoint config, got {len(self.normalized_policy_joint_names)}"
            )
        if self.normalized_policy_joint_names != JOINT_ORDER:
            print(
                "Warning: checkpoint action_feature_names order differs from dataset "
                f"JOINT_ORDER.\n  checkpoint: {self.policy_joint_names}\n"
                f"  normalized:{self.normalized_policy_joint_names}\n  dataset:   {JOINT_ORDER}"
            )
        ...
        robot_cfg = make_config(
            robot_id="xlerobot", use_cameras=not self.use_dashboard_cameras
        )
        self.robot = XLerobot(robot_cfg)
        _connect_robot_with_retries(self.robot)
        self.cam_session = None
        if self.use_dashboard_cameras:
            from _camera_preflight import run_camera_preflight

            run_camera_preflight(CONFIG_YAML)
            self.cam_session = attach_dashboard_cameras_to_robot(self.robot, warmup_s=3.0)
        print_inference_camera_map(self.cfg, self.policy)
        self.camera_preview = None
        if args.show_cameras:
            preview_src: Any = self.cam_session if self.use_dashboard_cameras else self.robot.cameras
            self.camera_preview = self.CameraPreviewWindow(
                preview_src, preview_fps=min(float(args.preview_fps), float(args.fps))
            )
            self.camera_preview.start()
        self.last_sent: dict[str, float] = {}
        self.last_filtered: dict[str, float] = {}
        self.last_policy_smoothed: dict[str, float] = {}
        self._stop_event = threading.Event()
        self.closed = False

    def request_stop(self, reason: str = "operator interruption") -> dict[str, Any]:
        """Request cooperative cancellation of the active task."""
        self._stop_event.set()
        return {"ok": True, "stop_requested": True, "reason": reason}
    ...
    def run_task(
        ...
        *,
        ...
        if self.closed:
            raise RuntimeError("PI0.5 runtime is closed")
        self._stop_event.clear()
        args = copy.copy(self.args)
        args.task = task
        if episodes is not None:
            args.episodes = max(1, int(episodes))
        if episode_time is not None:
            args.episode_time = max(1, int(episode_time))
        print(f"\nWarm runner task: {task}")
        total_steps = 0
        last_stop_reason = ""
        try:
            if self.home_pose and not args.home_before_episode:
                go_to_home_pose(
                    self.robot,
                    self.home_pose,
                    fps=float(args.fps),
                    timeout_s=float(args.home_timeout),
                    precise_sleep=self.precise_sleep,
                    caps=self.vr_joint_caps,
                    present_tolerance_deg=self.homing_present_tol,
                    soft_stall_tolerance_deg=self.homing_soft_stall_tol,
                )

            for ep in range(args.episodes):
                if self._stop_event.is_set():
                    last_stop_reason = "interrupted by operator"
                    break
                print(f"\n=== Episode {ep + 1}/{args.episodes} ===")
                episode_deadline = time.perf_counter() + float(args.episode_time)
                progress_log_t = 0.0
                if self.home_pose and args.home_before_episode:
                    pre_home_timeout = float(args.home_timeout)
                    if _episode_seconds_left(episode_deadline) < pre_home_timeout:
                        pre_home_timeout = max(
                            1.0, _episode_seconds_left(episode_deadline) - 1.0
                        )
                    go_to_home_pose(
                        self.robot,
                        self.home_pose,
                ...
                _reset_policy_episode_state(
                    ...
                    last_policy_smoothed=self.last_policy_smoothed,
                ...
                try:
                    while True:
                        ...
                        if step < settle_steps:
                            ...
                            sent_cmd = _send_positions(
                                self.robot, hold_cmd, present=present_dict
                            )
                            self.last_sent = dict(sent_cmd)
                            self.last_filtered = dict(sent_cmd)
                            step += 1
                            remaining = (1.0 / float(args.fps)) - (
                        ...
                        try:
                            ...
                            queue_empty_before = (
                                len(getattr(self.policy, "_action_queue", ())) == 0
                            )
                        ...
                        action_dict = _actions_to_robot_dict(
                            action, self.policy_joint_names
                        )
                        if args.policy_ema_alpha < 0.999:
                            policy_ref = (
                                self.last_policy_smoothed
                                if self.last_policy_smoothed
                                else action_dict
                            )
                            ...
                            self.last_policy_smoothed = dict(action_dict)
                        if (
                            queue_empty_before
                            and self.last_filtered
                        ...
                        final_cmd = _apply_joint_deadband(
                            final_cmd, self.last_filtered, self.vr_joint_deadbands
                        )
                        ...
                        if args.replan_on_miss_deg > 0 and self.last_filtered:
                            exec_err = _max_tracking_error_deg(
                                present_dict, self.last_filtered
                            )
                            miss_streak = (
                                miss_streak + 1
                            ...
                            if (
                                ...
                                and _flush_policy_action_queue(self.policy) > 0
                            ):
                                miss_streak = 0
                                miss_replans += 1
                        if not logged_action_debug:
                            ...
                            raw_cmd = np.array(
                                [action_dict[f"{n}.pos"] for n in JOINT_ORDER]
                            )
                            shaped_cmd = np.array(
                                [shaped[f"{n}.pos"] for n in JOINT_ORDER]
                            )
                            sent_cmd_arr = np.array(
                                [debug_final_cmd[f"{n}.pos"] for n in JOINT_ORDER]
                            )
                            ...
                            for name, delta in _top_joint_deltas(
                                debug_final_cmd, present_dict
                            ):
                        ...
                        try:
                            ...
                            sent_cmd = _send_positions(
                                self.robot, final_cmd, present=send_present
                            )
                        except Exception as exc:
                    ...
                    hold = dict(self.last_filtered or self.last_sent)
                    _reset_policy_episode_state(
                        self.policy,
                        self.preprocessor,
                        self.postprocessor,
                    ...
                    _safe_finish_episode(
                        ...
                        precise_sleep=self.precise_sleep,
                        caps=self.vr_joint_caps,
                        homing_present_tolerance_deg=self.homing_present_tol,
                        homing_soft_stall_tolerance_deg=self.homing_soft_stall_tol,
                        hold_cmd=hold if hold else None,
                        episode_deadline=episode_deadline,
                        skip_home_after=_skip_post_home_after_episode(args, stop_reason),
                    )
                    miss_note = (
                    ...
                    total_steps += step
                    last_stop_reason = stop_reason or ""
                    if self._stop_event.is_set():
                        break
        except KeyboardInterrupt:
            raise
    ...
    def close(self) -> None:
        ...
        if self.cam_session is not None:
            try:
                self.detach_dashboard_cameras_from_robot(self.robot)
            except Exception as exc:
