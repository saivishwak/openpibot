package com.openpibot.questteleop;
...
public final class RtpH264Receiver {
    ...
    public void start(int port, int width, int height) {
        ...
        lastError = "";
        frameCount = 0;
        receiveThread = new Thread(() -> runReceiver(port), "openpibot-rtp-h264-rx-" + port);
        receiveThread.start();
    }
    ...
    private void runReceiver(int port) {
        try {
            ...
            decoder.start();
            drainThread = new Thread(this::drainDecoder, "openpibot-h264-drain");
            drainThread.start();
