"""Use webapp CameraStream singletons for inference (same as VR teleop recording).

The webapp keeps long-lived V4L2 capture threads in a registry; VR recording reads
...
def _webapp_cameras_module() -> Any:
    root = str(REPO_ROOT)
    if root not in sys.path:
        sys.path.insert(0, root)
    from webapp.backend import cameras as webapp_cameras

    return webapp_cameras
...
class WebappCameraSession:
    ...
    def streams(self) -> dict[str, Any]:
        """Role name → webapp CameraStream (for preview; do not release)."""
    ...
    def start(self) -> None:
        if self._streams:
            return
        self._wc = _webapp_cameras_module()
        missing: list[str] = []
        ...
        if missing:
            raise RuntimeError(
                ...
                f"{missing}. Assign them on the webapp Cameras page."
            )

        print(
            "Tip: stop the webapp backend before inference — a second process "
        ...
        print(
            f"Webapp cameras ready ({len(self._streams)} streams, same path as VR teleop).\n"
            f"  {paths}"
        )
    ...
    def __enter__(self) -> WebappCameraSession:
        self.start()
        return self
...
def attach_webapp_cameras_to_robot(robot: Any, *, warmup_s: float = 3.0) -> WebappCameraSession:
    """Patch robot.get_observation to merge webapp camera frames with motor state."""
    session = WebappCameraSession(warmup_s=warmup_s)
    session.start()
    ...
    robot._webapp_camera_session = session  # type: ignore[attr-defined]
    return session


def detach_webapp_cameras_from_robot(robot: Any) -> None:
    session = getattr(robot, "_webapp_camera_session", None)
    if session is not None:
        session.stop()
        del robot._webapp_camera_session
